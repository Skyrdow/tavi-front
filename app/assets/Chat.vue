...
const sendMessage = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      const errorMessage: Message = { texto: "Error: " + response.message, ai: true };
      mensajes.value.push(errorMessage);
    }
  } catch (error) {
    const errorMessage: Message = { texto: "Error al conectar con la IA", ai: true };
    mensajes.value.push(errorMessage);
  } finally {
  ...
  >
    ...
    <div v-if="mediaFile" class="p-4 border-b bg-white">
      <div class="w-full max-w-sm mx-auto">
        <div class="aspect-square w-full bg-gray-200 flex items-center justify-center rounded-lg overflow-hidden">
          <img
            v-if="mediaUrl"
            :src="mediaUrl"
        ...
        <p class="text-center text-sm text-gray-600 mt-2">Vista previa de la imagen</p>
      </div>
    </div>
    ...
    <div class="flex-1 p-4 space-y-4 overflow-y-auto">
      ...
      <div v-if="isLoading" class="flex justify-start">
        <div class="max-w-[80%] p-3 rounded-xl shadow-md bg-gray-100 text-gray-800 rounded-tl-none">
