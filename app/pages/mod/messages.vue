...
const conversations = ref<Conversation[]>([
  {
    ...
    platform: 'WhatsApp',
    messages: [
      { id: 101, text: 'Hola, estoy interesado en el servicio de community manager. ¿Me pueden dar más detalles?', time: 'Hace 10 minutos', sender: 'client' },
      { id: 102, text: 'Claro, ¿cuáles son los precios?', time: 'Hace 8 minutos', sender: 'business' },
    ],
  ...
  },
  {
    ...
    messages: [
      { id: 201, text: '¿Cuál es el horario de atención?', time: 'Hace 3 horas', sender: 'client' },
      { id: 202, text: 'Nuestro horario de atención es de Lunes a Viernes de 9:00 a 18:00 hrs. ¡Estamos para servirte!', time: 'Hace 2 horas', sender: 'business' },
      { id: 203, text: 'Perfecto, gracias.', time: 'Hace 1 hora', sender: 'client' },
    ],
    suggestedResponse: '¡De nada! ¿Hay algo más en lo que podamos ayudarte hoy?',
    response: '',
    isResponsePending: true,
  },
]);
...
const selectConversation = (conversation: Conversation) => {
  ...
  if (!activeConversation.value.response) {
    activeConversation.value.response = activeConversation.value.suggestedResponse;
  }
};

const sendMessage = (conversation: Conversation) => {
  // 1. Mock API call to send message
  console.log(`Enviando mensaje a ${conversation.clientUser}: ${conversation.response}`);
...
<template>
  <div class="grid grid-cols-3 gap-6 min-h-[80vh]">
    ...
    <div class="col-span-1 bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
      <h2 class="text-xl font-bold p-4 border-b bg-gray-50 text-gray-800">Chats Pendientes ({{ totalPendingConversations }})</h2>
      <div class="divide-y divide-gray-100 max-h-[70vh] overflow-y-auto">
        <div
          ...
          :class="{ 'bg-indigo-100 border-l-4 border-indigo-500': activeConversation?.id === conversation.id }"
          @click="selectConversation(conversation)"
        >
          <div class="flex justify-between text-sm text-gray-500">
            <span class="font-semibold text-gray-700">{{ conversation.clientUser }}</span>
            <span class="text-xs">{{ conversation.messages[conversation.messages.length - 1].time }}</span>
          </div>
          <p class="mt-1 text-gray-800 truncate">{{ conversation.messages[conversation.messages.length - 1].text }}</p>
          <span class="text-xs font-medium text-indigo-600">{{ conversation.platform }}</span>
          <span v-if="conversation.isResponsePending" class="ml-2 text-xs font-bold text-red-500">¡PENDIENTE!</span>
        </div>
      </div>
    </div>

    <!-- Right Panel: Chat History and Response Editor -->
    <div v-if="activeConversation" class="col-span-2 bg-white rounded-xl shadow-lg border border-gray-200 flex flex-col">
      <h2 class="text-2xl font-bold p-4 border-b text-gray-800">Chat con {{ activeConversation.clientUser }} ({{ activeConversation.platform }})</h2>
      ...
      <div class="flex-1 p-6 space-y-4 overflow-y-auto max-h-[60vh]">
        <div 
          ...
          :key="message.id" 
          class="flex" 
          :class="{ 'justify-end': message.sender === 'business', 'justify-start': message.sender === 'client' }"
        >
          <div
            class="max-w-[80%] p-3 rounded-xl shadow-md"
            :class="{
              'bg-indigo-600 text-white rounded-br-none': message.sender === 'business',
              'bg-gray-100 text-gray-800 rounded-tl-none': message.sender === 'client',
            }"
          >
            <p class="text-xs font-semibold mb-1" :class="{ 'text-indigo-200': message.sender === 'business', 'text-gray-500': message.sender === 'client' }">
              {{ message.sender === 'business' ? 'Tú' : activeConversation.clientUser }}
            </p>
            <p>{{ message.text }}</p>
            <span class="block text-right text-xs mt-1" :class="{ 'text-indigo-300': message.sender === 'business', 'text-gray-500': message.sender === 'client' }">{{ message.time }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="p-6 border-t bg-gray-50 rounded-b-xl">
        <div v-if="activeConversation.isResponsePending" class="mb-4">
          ...
          <p class="mt-2 p-3 bg-indigo-100 border-l-4 border-indigo-400 text-gray-700 rounded-r-md text-sm">
