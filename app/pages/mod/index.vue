    ...
    comments: [
      {
        ...
        platform: "Instagram",
        suggestedResponse:
          "¡Hola! Nos alegra que te guste. Sí, tenemos 3 colores más. ¿Cuál te gustaría ver?",
        response: "",
        isReplySent: false,
      },
      {
        ...
        platform: "Instagram",
        suggestedResponse:
          "Entendemos tu preocupación. Actualmente tenemos una promoción del 15% por tiempo limitado. ¡Aprovecha!",
        response: "",
        isReplySent: false,
      },
    ],
    ...
    comments: [
      {
        ...
        platform: "Facebook",
        suggestedResponse:
          "¡De nada! Nos alegra que te sean útiles. ¡Síguenos para más tips!",
        response: "",
        isReplySent: false,
      },
    ],
...
const sendResponse = (comment: Comment) => {
  ...
  console.log(
    `Enviando respuesta a ${comment.user} en post ${activePost.value?.id}: ${comment.response}`,
  );
...
const totalPendingComments = mockPosts.value.reduce(
  (sum, post) => sum + post.comments.filter((c) => !c.isReplySent).length,
  0,
);
</script>
  ...
  <div class="grid grid-cols-3 gap-6 min-h-[80vh]">
    ...
    >
      <h2 class="text-xl font-bold p-4 border-b bg-gray-50 text-gray-800">
        Posts con Comentarios Pendientes ({{ totalPendingComments }})
      </h2>
      <div class="divide-y divide-gray-100 max-h-[70vh] overflow-y-auto">
        ...
        >
          <img
            ...
            class="size-12 object-cover rounded-md border"
            alt="Post Thumbnail"
          />
    ...
    <div
      v-if="activePost"
      class="col-span-2 bg-white rounded-xl shadow-lg p-6 border border-gray-200 flex flex-col"
    >
      ...
      <div
        class="flex items-center gap-3 mb-4 p-3 bg-gray-50 rounded-lg border border-gray-200"
      >
        <img
          ...
          alt="Post Thumbnail"
        />
        <p class="text-sm text-gray-700 italic truncate">
          {{ activePost.caption }}
        </p>
      </div>
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="bg-gray-100 p-3 rounded-xl rounded-tl-none">
              <p class="font-semibold text-sm text-gray-900 inline mr-1">
                {{ comment.user }}
              </p>
            ...
            <!-- Actions/Meta -->
            <div
              class="flex items-center gap-3 mt-1 text-xs text-gray-500 ml-1"
            >
            ...
            <div
              ...
              class="mt-3 p-4 bg-white rounded-lg shadow-inner border border-indigo-200"
            >
              <h3
